...
export async function registerDocumentTemplate(formData: FormData): Promise<void> {
  ...
  if (!name || !docType) {
    throw new Error('Nombre y tipo de documento son requeridos');
  }

  const file = formData.get('template') as File;
  if (!file || file.size === 0) {
    throw new Error('Archivo de plantilla requerido');
  }
 ...
 */
export async function generateDocxFromTemplate(templateId: string, data: Record<string, any>) {
  ensureTemplatesDir();
  ...
  if (template.docType !== 'docx') {
    throw new Error('La plantilla no es de tipo docx');
  }

  const filePath = join(TEMPLATES_DIR, template.path);
  if (!existsSync(filePath)) {
    throw new Error('Archivo de plantilla no encontrado en disco');
  }
