...
### Medium Priority
- [ ] **Riesgo de prompt injection vía mensajes `role: 'user'` en el chat del asistente** (`app/api/chat/route.ts`, Sprint 6.2, reviewer). El servidor descarta cualquier mensaje `role: 'system'` que el cliente intente mandar (mitigado), pero un mensaje de usuario tipo "ignora las instrucciones anteriores y..." no está ni puede estar mitigado con el patrón actual — es un problema general de cualquier chat con LLM, no específico de EMA OS. Hoy (Sprint 6.2, solo lectura de contexto) el impacto es bajo. **Importa mucho más a partir de Sprint 6.4** (tool-use de escritura: crear_tarea, completar_tarea, mover_archivo_a_proyecto) porque ahí una inyección exitosa podría intentar disparar una tool real — el flag de confirmación explícita del usuario (ya exigido por diseño en `AGENTES.md` y en el pedido de Sprint 6.4) es la mitigación principal, pero vale revisar explícitamente este vector al implementar 6.4, no darlo por cubierto implícitamente.
