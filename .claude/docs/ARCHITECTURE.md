...
├── app/lib/prisma/   # Generated Prisma Client (solo output, nunca editar a mano)
├── app/lib/db.ts     # Singleton exportado `prisma` (fuera del output, sobrevive a `prisma generate`)
```